...
class KernelBuilder:
    ...
    def build_kernel(
        ...
        shift_31_const = self.scratch_const(31)
        ...
        shift_31_vec = self.alloc_scratch("shift_31", VLEN)
        ...
        body.append({"load": [("vload", idx_chunks[0], addr_tmp),
                     ...
                     "valu": [("vbroadcast", val1_vec_4, val1_const_4),
                              ...
                              ("vbroadcast", shift_31_vec, shift_31_const)]})
        ...
        for round_num in range(rounds):
            for c in range(n_chunks):
                ...
                body.append({"valu": [("<", hash_t1s[0], idx_chunks[c], n_nodes_vec)]})
                if c + 1 < n_chunks:
                    body.append({"flow": [("vselect", idx_chunks[c], hash_t1s[0], idx_chunks[c], zero_vec)],
                                 "valu": [("+", addr_vecs[c+1], forest_p_vec, idx_chunks[c+1])]})
                elif round_num < rounds - 1:
                    body.append({"flow": [("vselect", idx_chunks[c], hash_t1s[0], idx_chunks[c], zero_vec)],
                                 "valu": [("+", addr_vecs[0], forest_p_vec, idx_chunks[0])]})
                else:
                    body.append({"flow": [("vselect", idx_chunks[c], hash_t1s[0], idx_chunks[c], zero_vec)],
                                 "alu": [("+", addr_tmp, self.scratch["inp_indices_p"], i_const_0),
                                         ("+", addr_tmp2, self.scratch["inp_values_p"], i_const_0)]})
